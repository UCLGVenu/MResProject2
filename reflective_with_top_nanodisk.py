...
#Assumes perfect alignment despite nanodisk
# Assumes Al reflective layer, with refractive index < LC - no phase shift on reflection
# Thus simply double pass through LCs, and apply absorption losses twice and add phase twice.
...
output = np.matmul(matrix, input_jones)
print(output)
output_x_int2 = (output[0].real ** 2 + output[0].imag ** 2) ** 0.5
output_x_phs2 = np.arctan(output[0].imag / output[0].real)

output_x_int2 *= (1 - calc_antenna_absorb(bot_wav_avg))
output_x_phs2 += calc_anteanna_phase(bot_wav_avg)

output_x_int2 *= (1 - calc_antenna_absorb(bot_wav_avg))
output_x_phs2 += calc_anteanna_phase(bot_wav_avg)

output_x_phs_rem = output_x_phs2 % (2 * np.pi)

print(output_x_int2, output_x_phs2, output_x_phs_rem)
